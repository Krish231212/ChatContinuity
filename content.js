/**
 * ChatContinuity - Content Script Entrypoint
 * Dynamically loads and runs ES module dependencies in pages
 */

(async () => {
  try {
    // 1. Resolve internal URLs of our ES modules
    const storageUrl = chrome.runtime.getURL('storage.js');
    const continuationUrl = chrome.runtime.getURL('continuation.js');
    const domExtractorUrl = chrome.runtime.getURL('domExtractor.js');
    const selectorManagerUrl = chrome.runtime.getURL('selectorManager.js');
    const memoryManagerUrl = chrome.runtime.getURL('memoryManager.js');
    const utilsUrl = chrome.runtime.getURL('utils.js');

    // 2. Import modules
    const { getContinuationState, saveContinuationState, clearAllSessionData } = await import(storageUrl);
    const { isFreshChatPage } = await import(continuationUrl);
    const { pasteContinuationPrompt } = await import(domExtractorUrl);
    const { detectPlatform } = await import(selectorManagerUrl);
    const { initializeMemoryManager } = await import(memoryManagerUrl);
    const { logger, delay } = await import(utilsUrl);
    ...
    console.error('[ChatContinuity] Content script error during dynamic loading:', err);
  }
})();
